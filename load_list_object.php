...
for ($x = 0; $x < count($data2); $x++) {
    if ($data3[$x] == null && $data1[$x] == $input) {
        ...
        echo "<input class='form-check-input' style='display:inline;' type='checkbox' id='inlineCheckbox".$data1[$x]."' value='".$data1[$x]."'>";
...
function sub_object($searching, $dat1, $dat2, $dat3, $dat4, $find2, $look, $input)
{
    ...
    for ($i = 0; $i < count($dat2); $i++) {
        if ($searching == $dat4[$i]) {
            ...
            echo "<div class='form-check form-check-inline'>";
            echo "<input class='form-check-input' style='display:inline;' type='checkbox' id='inlineCheckbox".$dat1[$i]."' value='".$dat1[$i]."'>";
