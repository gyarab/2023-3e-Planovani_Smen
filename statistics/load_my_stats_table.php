...
for ($i = 1; $i < 32; $i++) {
    ...
    if (mysqli_num_rows($check_get) == 0) {
        ...
        while ($rows = $result->fetch_assoc()) {
            ...
            echo '<div class="col-12 col-md-3">';
            echo '<div style="display:inline">';
            echo '<p id="l' . $count . '" style="display:inline">' . $time . '</p>' . $not_unlog . '';
            echo '</div>';
            echo '<div style="display:inline">';
            echo ' <p style="display:inline">&nbsp;-&nbsp;&nbsp;' . round((int) $time) . 'h&nbsp;' . ((int) ($time_row / 60)) . 'min' . $not_unlog . '</p>';
            echo '</div>';
            echo '</div>';
            echo '<div class="col-12 col-md-2">';
            echo '<div style="display:inline">';
            echo '<p id="lr' . $count . '" style="display:inline">' . ((round($time * 4)) / 4) . '</p>' . $not_unlog . '';
            echo '</div>';
            echo '<div style="display:inline">';
            echo ' <p style="display:inline">&nbsp;-&nbsp;&nbsp;' . round((int) ((round($time * 4)) / 4)) . 'h&nbsp;' . (int) ((((round($time * 4)) / 4) * 3600) % 3600 / 60) . 'min' . $not_unlog . '</p>';
